...
class PlayState extends MusicBeatState {
	...
	override public function create() {
		...
		if (ClientPrefs.data.hitsoundVolume > 0)
			Paths.sound('gameplay/hitsound');
		for (i in 1...4)
			Paths.sound('gameplay/missnote/missnote$i');
		Paths.image('alphabet/alphabet');
	...
	function cacheCountdown() {
		...
		for (asset in introAlts)
			Paths.image('gameplay_UI/count/$asset');

		Paths.sound('gameplay/count/intro3' + introSoundsSuffix);
		Paths.sound('gameplay/count/intro2' + introSoundsSuffix);
		Paths.sound('gameplay/count/intro1' + introSoundsSuffix);
		Paths.sound('gameplay/count/introGo' + introSoundsSuffix);
	}
	...
	public function startCountdown() {
		...
		if (ret != LuaUtils.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
				...
				switch (swagCounter) {
					case 0:
						FlxG.sound.play(Paths.sound('gameplay/count/intro3' + introSoundsSuffix), 0.6);
						tick = THREE;
					case 1:
						countdownReady = createCountdownSprite(introAlts[0], antialias);
						FlxG.sound.play(Paths.sound('gameplay/count/intro2' + introSoundsSuffix), 0.6);
						tick = TWO;
					case 2:
						countdownSet = createCountdownSprite(introAlts[1], antialias);
						FlxG.sound.play(Paths.sound('gameplay/count/intro1' + introSoundsSuffix), 0.6);
						tick = ONE;
					case 3:
						countdownGo = createCountdownSprite(introAlts[2], antialias);
						FlxG.sound.play(Paths.sound('gameplay/count/introGo' + introSoundsSuffix), 0.6);
						tick = GO;
					case 4:
						tick = START;
				}
	...
	inline private function createCountdownSprite(image:String, antialias:Bool):FlxSprite {
		var spr:FlxSprite = new FlxSprite().loadGraphic(Paths.image('gameplay_UI/count/$image'));
		spr.cameras = [camHUD];
	...
	public dynamic function updateScore(miss:Bool = false) {
		...
		if (ClientPrefs.data.comboDisplay == "Info Bar") {
			tempScore = 'Score: ${songScore}' + (!instakillOnMiss ? ' | Misses: ${songMisses}' : "") + ' | Rating: ${str} | Combo = ${combo}';
		}
	...
	public var showComboNum:Bool = ClientPrefs.data.comboDisplay == 'Screen';
	...
	private function cachePopUpScore() {
		...
		for (rating in ratingsData)
			Paths.image('gameplay_UI/ratings/' + uiPrefix + rating.image + uiSuffix);
		for (i in 0...10)
			Paths.image('gameplay_UI/num/' + uiPrefix + 'num' + i + uiSuffix);
	}
	...
	private function popUpScore(note:Note = null):Void {
		...
		rating.loadGraphic(Paths.image('gameplay_UI/ratings/' + uiPrefix + daRating.image + uiSuffix));
		rating.screenCenter();
		var offsetX = ClientPrefs.data.middleScroll ? FlxG.width / 2 - 200 : 0;
		rating.x = ((gf.getGraphicMidpoint().x) - (rating.width / 2)) + offsetX;
		rating.y = ((gf.getGraphicMidpoint().y) - (rating.height));
		...
		for (i in seperatedScore) {
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image('gameplay_UI/num/' + uiPrefix + 'num' + Std.int(i) + uiSuffix));
			numScore.screenCenter();
			var offsetX = ClientPrefs.data.middleScroll ? FlxG.width / 2 - 80 : 0;
			numScore.x = ((gf.getGraphicMidpoint().x) - (numScore.width / 2)) + offsetX + (43 * daLoop);
			numScore.y = rating.y = ((gf.getGraphicMidpoint().y) + (numScore.height + 10));
