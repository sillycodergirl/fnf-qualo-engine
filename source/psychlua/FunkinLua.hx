...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "makeGraphic", function(obj:String, width:Int = 256, height:Int = 256, color:String = 'FFFFFF', oldMethod:Bool = false) {
			var spr:FlxSprite = LuaUtils.getObjectDirectly(obj, false);
			if (spr != null) {
				if (oldMethod)
					spr.makeGraphic(width, height, CoolUtil.colorFromString(color));
				else { // better mem usage
		...
		Lua_helper.add_callback(lua, "superMakeLuaSprite",
			function(tag:String, isAnimated:Bool = false, image:String = null, front:Bool = true, ?x:Float = 0, ?y:Float = 0, ?scaleX:Float = 1,
					?scaleY:Float = 1, ?updateHitbox:Bool = true, ?scrollX:Float = 1, ?scrollY:Float = 1, ?cameras:String = '', ?alpha:Float = 1,
					?velocityX:Float = 0, ?velocityY:Float = 0, ?zIndex:Int = 1, ?precache:Bool = false, ?center:String = null) {
				...
				if (image != null && image.length > 0) {
					if (isAnimated) {
						LuaUtils.loadFrames(leSprite, image, 'sparrow');
					} else {
						leSprite.loadGraphic(Paths.image(image));
					}
				}
				...
				// scaleObject()
				leSprite.scale.set(scaleX, scaleY);
				...
				// setScrollFactor()
				leSprite.scrollFactor.set(scrollX, scrollY);

				// setObjectCamera()
				leSprite.cameras = [LuaUtils.cameraFromString(cameras)];

				// no function but precaching shit
				if (precache)
					leSprite.alpha = 0.00001;
				else
					leSprite.alpha = alpha;

				// no function but velocity
				leSprite.velocity.set(velocityX, velocityY);

				// setObjectOrder()
				LuaUtils.getTargetInstance().remove(leSprite, true);
				LuaUtils.getTargetInstance().insert(zIndex, leSprite);

				// screenCenter()
				if (center != null) {
					switch (center.trim().toLowerCase()) {
						case 'x':
							leSprite.screenCenter(X);
						case 'y':
							leSprite.screenCenter(Y);
						case 'xy':
							leSprite.screenCenter(XY);
					}
				}
